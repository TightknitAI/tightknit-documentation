...
You have the opportunity to create a Feed during the guided setup to [enable a channel](/channels-and-feeds/enable-channel#enable-a-channel-for-tightknit). You can also create a Tightknit Feed for enabled Slack channel by clicking the `Create Tightknit Feed` option within the row-level menu of the channel settings.
...
- **Message Sync Settings** - By default, Tightknit will sync all incoming new Slack messages and replies to the Feed, starting from when this setting is enabled. If you do _not_ want to auto-sync messages (e.g., you prefer to manually sync select posts using the [shortcut](../basics/message-shortcuts#tightknit-shortcuts)), uncheck this option.
