...
<Callout type="info">
  By default, only Admins can create events. If you would like to allow
  non-Admins to create events, you can do so by adding users to a
  [Group](/community/groups) which is assigned a [Permission
  Set](/community/permission-sets) with the Event Champion permissions enabled.
</Callout>
...
Within the Publish Settings, you will designate the privacy and audience settings of the event. The _Privacy Level_ determines which users in the Slack workspace can discover or access the event:

- **Public**: accessible by all users in the workspace, via the [Events List panel](/events/getting-started#events-list-panel)
- **Unlisted**: accessible only by members of the selected channel(s). Certain users will still see unlisted events in the Events List panel ([read more](/events/getting-started#views)).

You will select the Slack channel(s) that will receive the [event announcement](#published-event-announcement). You may select up to 10 channels.

You may also choose to make the event publicly shared on the companion site. The event will appear in the Events list as well as on the Feed page for any Feeds that are connected to your selected Slack channels.
Here is an example event on the companion site:
...
When you provide a [Luma](https://lu.ma/) URL, like `https://lu.ma/event/evt-XYZ123`, for the **Event Link** of a Tightknit event, we provide an integrated 1-click registration experience within the Event Page on the companion site.

![Events Luma integration](/events/events-luma-integration.png)
